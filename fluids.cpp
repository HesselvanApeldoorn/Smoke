...
#define NROPAQUE 10 // For number of opaque spinner in glui
...
void Fluids::move_camera(int key, int x, int y)
{
    int change =  2.5;
    switch(key)
    {
        case GLUT_KEY_LEFT: 
            camera_heading += change;
        break;
        case GLUT_KEY_RIGHT: 
            camera_heading -= change;
        break;
...
void Fluids::display(void)
{
    ...
    glPushMatrix(); //Push and do transformations (will be popped before legend)

    glTranslatef((winWidth-Fluids::GUI_WIDTH)/2,winHeight/2,0);
    glRotatef(camera_pitch, 0, 1, 0);
    glRotatef(camera_heading, 1, 0, 0);
    glTranslatef(-(winWidth-Fluids::GUI_WIDTH)/2,-winHeight/2,0);
...

{
    ...
    glLoadIdentity();
    // gluPerspective(00.0,(GLdouble)w/(GLdouble)h,1.5,9001);

    glOrtho(0.0, (GLdouble)w, 0.0, (GLdouble)h, -10000,10000);
